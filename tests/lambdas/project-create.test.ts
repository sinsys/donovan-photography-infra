...
describe('createProject', () => {
  ...
  it('creates project successfully', async () => {
    ...
    const expected = mockCreateProjectResponse
    const consoleSpy = jest.spyOn(console, 'error').mockImplementation(() => {})

    // Act
    const result = await createProject(mockCreateProjectRequest, mockContext.object(), () => {})
  ...
  it('logs api error when error is known', async () => {
    ...
    // Act / Assert
    await expect(createProject(input, mockContext.object(), () => {}))
      .rejects
      .toThrow(expected)
  ...
  it('logs unknown error when error is unknown', async () => {
    ...
    // Act / Assert
    await expect(createProject(mockCreateProjectRequest, mockContext.object(), () => {}))
      .rejects
      .toThrow(expected)
    expect(consoleSpy).toHaveBeenCalledWith(...expectedLogs)
    consoleSpy.mockRestore()
